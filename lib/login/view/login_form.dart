...
class _TwitterLoginButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ElevatedButton.icon(
      ...
      onPressed: () async {
        var _authPage = await context.read<LoginCubit>().getTwitterAuthPage();
        var _userInfo =
            await Navigator.of(context).push<TwitterUserInfo>(MaterialPageRoute(
          builder: (context) => TwitterLoginWebView(_authPage ?? ''),
        ));
        var test = _userInfo?.oauthprovider ?? '';
        await context
            .read<LoginCubit>()
